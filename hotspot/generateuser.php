...
// echo "<h2>🔹 Data SESSION</h2><pre>";
// print_r($_SESSION);
// echo "</pre>";

// // Debug semua variabel GET
// echo "<h2>🔹 Data GET</h2><pre>";
// print_r($_GET);
// echo "</pre>";
...
<?php if ($_SESSION['ubp'] == "") {
					...
					<table class="table">
						...
						<!-- <tr>
							...
							<td>
								<div class="input-group">
									<div class="input-group-10 col-box-9">
										<input class="group-item group-item-l form-control" type="number" name="price" value="">
									</div>
									<div class="input-group-2 col-box-3">
						...
						<tr>
							...
							<td><input class="form-control " type="text" title="No special characters" id="email" autocomplete="off" name="adcomment" value=""></td>
						</tr>
						<tr>
							<td class="align-middle">Price</td>
							<td><input class="form-control " type="text" title="No special characters" id="pricebaru" autocomplete="off" name="adcomment" value=""></td>
						</tr>
						<tr>
	...
	<div class="col-4">
		...
		<button id="checkStorage" class="btn btn-info">Cek Data Agent</button>
	</div>
	<script>
		// get valid $ price
		function GetVP() {
		...
		document.addEventListener("DOMContentLoaded", function() {
			const authUser = JSON.parse(localStorage.getItem("authUser")); // Ambil data user dari localStorage
			const token = authUser ? authUser.token : null; // Ambil token dari authUser
			const apiUrl = "http://voucher.jinom.net/api/agents";
			const agentDropdown = document.getElementById("dropdown");
			const commentInput = document.querySelector("input[name='adcomment']");
			const checkStorageButton = document.getElementById("checkStorage"); // Tombol cek storage

			if (!token) {
				console.error("Token tidak ditemukan. Silakan login kembali.");
				agentDropdown.innerHTML = '<option value="">Token tidak ditemukan. Silakan login ulang.</option>';
				return;
			}
			...
			fetch(apiUrl, {
				...
				.then((data) => {
					...
					defaultOption.value = "";
					defaultOption.textContent = "Pilih Agen...";
					...
					data.forEach((agent) => {
						...
						option.textContent = `${agent.id} - ${agent.nama || "Tanpa Nama"} - ${agent.nomer_hp}`;
						option.dataset.agentId = agent.id; // Simpan agent_id di dataset
						agentDropdown.appendChild(option);
					});


					const savedAgent = localStorage.getItem("selectedAgent");
					const savedAgentEmail = localStorage.getItem("selectAgentEmail")
					const savedAgentId = localStorage.getItem("selectedAgentId");

					if (savedAgent && savedAgentId) {
						agentDropdown.value = savedAgent; // Atur dropdown ke nilai yang tersimpan
						commentInput.value = savedAgent; // Isi input dengan nama agent
					}
				})
				.catch((error) => {
					console.error("Error fetching data:", error);
					agentDropdown.innerHTML = '<option value="">Gagal mengambil data agen.</option>';
				});


			agentDropdown.addEventListener("change", function() {
				const selectedOption = agentDropdown.options[agentDropdown.selectedIndex];
				const selectedAgent = selectedOption.value;
				const selectedAgentId = selectedOption.dataset.agentId; // Ambil `agent_id` dari dataset
				const selectAgentEmail = selectedOption.dataset.email;
				commentInput.value = selectedAgent; // Isi input dengan nama agen


				localStorage.setItem("selectedAgent", selectedAgent);
				localStorage.setItem("selectedAgentId", selectedAgentId);
				localStorage.setItem("selectAgentEmail", selectedAgentEmail);
				sessionStorage.setItem("selectedAgent", selectedAgent);
				sessionStorage.setItem("selectedAgentId", selectedAgentId);
				sessionStorage.setItem("selectAgentEmail", selectAgentEmail);

				console.log("Agent tersimpan:", selectedAgent, "ID:", selectedAgentId, "email:", selectAgentEmail);


				if (selectedAgent) {
					alert(`Anda memilih agen: ${selectedAgent}\nAgent ID: ${selectedAgentId} \nAgent Email: ${selectAgentEmail}`);
				}
			});


			checkStorageButton.addEventListener("click", function() {
				const localAgent = localStorage.getItem("selectedAgent") || "Tidak ada";
				const localAgentId = localStorage.getItem("selectedAgentId") || "Tivdak ada";
				const sessionAgent = sessionStorage.getItem("selectedAgent") || "Tidak ada";
				const sessionAgentId = sessionStorage.getItem("selectedAgentId") || "Tidak ada";
				const localToken = localStorage.getItem("authUser") || "Tidak ada";
				console.log("authUser:", localToken);

				console.log("Local Storage:", localAgent, "ID:", localAgentId);
				console.log("Session Storage:", sessionAgent, "ID:", sessionAgentId);
				alert(`Data tersimpan:\nLocal Storage: ${localAgent} (ID: ${localAgentId})\nSession Storage: ${sessionAgent} (ID: ${sessionAgentId})(TOKEN: ${localToken})`);
			});
		});


		document.addEventListener("DOMContentLoaded", async function() {
			console.log("🔹 Memuat data dari PHP...");

			try {
				const response = await fetch("proses.php", {
					method: "POST", // Kirim request ke PHP
					headers: {
						"Content-Type": "application/json"
					},
					body: JSON.stringify({})
				...
				// Simpan data ke localStorage untuk digunakan nanti
				localStorage.setItem("phpData", JSON.stringify(phpData));

			} catch (error) {
				console.error("❌ Gagal mengambil data PHP:", error);
			}
		});
	</script>
</div>