...
<div class="card-header">
    <h3><i class="fa fa-edit"></i><?php  echo $_edit_user.' '.$uname.' '; if ($utimelimit == "1s") {  echo $_expired;}?>INI DIA</h3>
</div>
<?php 
echo "<pre>";
var_dump($_POST);  // Jika data dikirim dengan metode POST
var_dump($_GET);   // Jika data dikirim dengan metode GET
echo "</pre>"
...
<form autocomplete="new-password" method="post" action="">
  <div>
    ...
    <?php 
    $hotspotUser = isset($_GET['hotspot-user']) ? $_GET['hotspot-user'] : null;
    $session = isset($_GET['session']) ? $_GET['session'] : null;
    
    echo "Hotspot User: " . $hotspotUser . "<br>";
