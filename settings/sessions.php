                          ...
                          <div class="box-group">
                            ...
                            <div class="box-group-area">
                              <span>
                                ...
                                <a href="./admin.php?id=settings&session=<?= $value; ?>">
                                  <i class="fa fa-edit"></i> <?= $_edit ?>
                                </a>&nbsp;
                                <a href="javascript:void(0)" onclick="if(confirm('Are you sure to delete data <?= $value;
                                  echo " (" . explode('%', $data[$value][4])[1] . ")"; ?>?')){loadpage('./admin.php?id=remove-session&session=<?= $value; ?>')}else{}">
                                  <i class="fa fa-remove"></i> <?= $_delete ?>
                                </a>
                              </span>
                            </div>
                          </div>
          ...
          <div class="col-6">
            <form autocomplete="off" method="post" action="">
              <div class="card">
                ...
                <div class="card-body">
                  <p><strong>Login sebagai:</strong> <span id="user-info">Memuat...</span> (<span id="user-email">Memuat...</span>)</p>
                  <table class="table table-sm">
                    <tr>
                      <td class="align-middle"><?= $_user_name ?> </td>
                      <td>
                        <input class="form-control" id="useradm" type="text" size="10" name="useradm" title="User Admin" value="<?= $useradm; ?>" required="1" />
                      </td>
                    </tr>
                    <tr>
...
document.addEventListener("DOMContentLoaded", function() {
    const authUser = JSON.parse(localStorage.getItem('authUser')); // Ambil dari localStorage
    const userHeader = document.getElementById("user-header"); // Header utama
    const userInfo = document.getElementById("user-info"); // Elemen untuk user
    const userEmail = document.getElementById("user-email"); // Elemen untuk email

    // Menampilkan User & Email
    if (authUser) {
        userHeader.textContent = `USER: ${authUser.user || "Guest"}`;
        userInfo.textContent = authUser.user || "Guest";
        userEmail.textContent = authUser.email || "Tidak Diketahui";
    } else {
        userHeader.textContent = "User Guest Setting";
        userInfo.textContent = "Guest";
        userEmail.textContent = "Tidak Diketahui";
    }
});
</script>
